...
import { isCompositeAnimation, getCompositeAnimation, getComponentAnimations, getDefaultAnimation as getDefaultAnimationFromMappings, cutsceneComposites } from '@/utils/json/cutscene_mappings'
import { buildCompositeSchedule, applySegmentsToState, type CompositeSegment, getAnimDuration } from '@/utils/compositeAnimations'
...
const handleActionStart = () => {
  ...
  // DISABLED: Aim mode click handling for testing
  if (market.live2d.current_pose === 'aim') {
    console.log('Click disabled for aim mode - use motion list to test')
    return
  }
...
// Handle when spine player UI tries to play an animation
const setupSpinePlayerAnimationListener = (player: any) => {
  if (!player || !player.setAnimation) return
  
  // Store the original setAnimation method
  const originalSetAnimation = player.setAnimation.bind(player)
  
  // Override setAnimation to intercept composite animations
  player.setAnimation = function(animationName: string, loop: boolean = true) {
    console.log('=== setAnimation called ===')
    console.log('Animation name:', animationName)
    console.log('Loop:', loop)
    console.log('Current character ID:', market.live2d.current_id)
    console.log('Is composite?:', isCompositeAnimation(market.live2d.current_id, animationName))
    
    // Check if it's a composite animation
    if (isCompositeAnimation(market.live2d.current_id, animationName)) {
      console.log('✓ INTERCEPTING composite animation:', animationName)
      ...
      } else if (market.live2d.current_id === 'c994' || market.live2d.current_id === 'c996') {
        idleAnimation = 'idle_02'
      }
      
      console.log('Calling playCompositeAnimation')
      // Play composite animation
      playCompositeAnimation(animationName)
      return
    }
    
    console.log('✗ Playing as regular animation:', animationName)
    // For regular animations, call the original method
    originalSetAnimation(animationName, loop)
  }
}
    ...
    frURL.onload = () => {
      ...
      spineCanvas = new usedSpine.SpinePlayer('player-container', {
        ...
        success: (player: any) => {
          ...
          // Add composite animations to the skeleton's animation list so they appear in the UI
          const compositeAnimList = cutsceneComposites[market.live2d.current_id]
          let compositeAnimations: string[] = []
          
          if (Array.isArray(compositeAnimList)) {
            compositeAnimations = compositeAnimList.map(c => c.name || '')
          } else if (compositeAnimList) {
            compositeAnimations = Object.keys(compositeAnimList)
          }
          
          if (compositeAnimations.length > 0) {
            console.log(`Adding ${compositeAnimations.length} composite animations to skeleton:`, compositeAnimations)
            compositeAnimations.forEach(compositeName => {
              // Create a fake animation object for composite animations
              player.animationState.data.skeletonData.animations.push({
                name: compositeName,
                duration: 0,
                timelines: []
              })
          ...
          // Setup listener for when UI tries to play composite animations
          setupSpinePlayerAnimationListener(player)
          ...
          const isComposite = isCompositeAnimation(market.live2d.current_id, currentAnimation)
          const animationExists = animations.some((a: { name: string }) => a.name === currentAnimation)
          
          if (!animationExists && !isComposite) {
            // Fallback to 'idle' if current animation doesn't exist and it's not a composite
            ...
            // For composite animations, set it to empty initially - they'll be triggered on click
...
const getDefaultAnimationForLoader = (availableAnimations?: Array<{ name: string }>) => {
  ...
  switch (market.live2d.current_pose) {
    case 'aim':
      // For aim pose, start with a real animation, not the composite
      // The composite will be triggered when user clicks
      return 'cut_A'
    case 'cover':
      return 'idle1'
    default:
...
let compositeAnimations: Array<{ name: string; hold?: boolean }> = []
let compositeDuration = 0
let compositeElapsed = 0
let compositeActive = false
let compositeRenderHandle: number | null = null
let compositeLastTimestamp: number | null = null

/**
 * Stop the spine-player's built-in render loop
 */
const stopSpinePlayerRenderLoop = (p: any) => {
  if (!p) return
  ;(p as unknown as { stopRequestAnimationFrame?: boolean }).stopRequestAnimationFrame = true
}

/**
 * Start the spine-player's built-in render loop
 */
const startSpinePlayerRenderLoop = (p: any) => {
  if (!p) return
...
/**
 * Stop composite rendering and restart spine-player loop
 */
const stopCompositeRender = () => {
  if (compositeRenderHandle !== null) {
    cancelAnimationFrame(compositeRenderHandle)
    compositeRenderHandle = null
  }
  compositeLastTimestamp = null
  compositeActive = false
  
  // Restart the spine-player's render loop
  if (spinePlayer) {
    startSpinePlayerRenderLoop(spinePlayer)
  }
}

/**
 * Render loop for composite animations - respects schedule timing
 */
/**
 * Render loop for composite animations - plays all animations in parallel
 */
const renderCompositeFrame = (timestamp: number = 0) => {
  if (!spinePlayer || !compositeActive) return

  // Track delta time
  if (compositeLastTimestamp === null) {
    compositeLastTimestamp = timestamp
  }
  
  const deltaTime = Math.min((timestamp - compositeLastTimestamp) / 1000, 0.016) // Cap at 60fps
  compositeLastTimestamp = timestamp
  compositeElapsed += deltaTime

  // Find all animations that should be playing right now
  const activeSegments = compositeSchedule.filter(seg => {
    const start = seg.start || 0
    const end = start + seg.duration
    return compositeElapsed >= start && compositeElapsed < end
  })

  // Clear all tracks
  spinePlayer.animationState.clearTracks()
  
  // Set each active animation on its own track for true parallel playback
  activeSegments.forEach((seg, trackIndex) => {
    const segmentElapsed = Math.max(0, compositeElapsed - seg.start)
    const entry = spinePlayer.animationState.setAnimation(trackIndex, seg.name, false)
    if (entry) {
      entry.mixDuration = 0
      entry.mixTime = 0
  ...
  // Draw frame
  const internal = spinePlayer as unknown as { drawFrame?: (requestNextFrame?: boolean) => void }
  if (internal.drawFrame) {
    internal.drawFrame(false)
  }

  // Check if finished
  if (compositeDuration > 0 && compositeElapsed >= compositeDuration) {
    compositeActive = false
    compositeRenderHandle = null
    return
  }
  ...
  // Continue loop
  compositeRenderHandle = requestAnimationFrame(renderCompositeFrame)
}

/**
 * Play a composite animation - all animations play simultaneously
 */
const playCompositeAnimation = (compositeAnimationName: string) => {
  if (!spinePlayer) return

  const composite = getCompositeAnimation(market.live2d.current_id, compositeAnimationName)
  if (!composite || !composite.composite) {
    console.warn(`No composite found for '${compositeAnimationName}'`)
    return
  }

  console.log(`Playing composite '${compositeAnimationName}' in PARALLEL`)

  // Stop any existing composite rendering
  stopCompositeRender()

  // Build schedule to understand timing
  const { schedule, duration } = buildCompositeSchedule(composite.composite, spinePlayer.animationState)
  
  if (schedule.length === 0) {
    console.warn(`No animations in schedule for '${compositeAnimationName}'`)
    return
  }

  console.log(`Schedule: ${schedule.length} segments, total duration ${duration.toFixed(2)}s`)
  schedule.forEach((seg, i) => {
    console.log(`  [${i}] ${seg.name}: starts ${seg.start.toFixed(2)}s, duration ${seg.duration.toFixed(2)}s, track ${seg.track}`)
  })

  // Stop spine-player's render loop
  stopSpinePlayerRenderLoop(spinePlayer)

  // Store for render loop - DON'T pre-set tracks, let render loop handle it
  compositeSchedule = schedule
  compositeDuration = duration
  compositeElapsed = 0
  compositeActive = true

  // Start custom render loop that handles parallel playback
  compositeLastTimestamp = null
  compositeRenderHandle = requestAnimationFrame(renderCompositeFrame)
}
...
const handleAction = () => {
  ...
  if (isCompositeAnimation(market.live2d.current_id, actionAnimation)) {
    playCompositeAnimation(actionAnimation)
    playVoice()
    return
  }
