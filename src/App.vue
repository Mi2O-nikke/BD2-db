...
function onHitAreaToggle(visible: boolean) {
  ...
  try {
    // Try to call setHitAreaVisible if it exists
    if (typeof (viewerRef.value as any).setHitAreaVisible === 'function') {
      (viewerRef.value as any).setHitAreaVisible(visible)
    } else {
      console.warn('SpineViewer does not have setHitAreaVisible method yet')
    }
  } catch (error) {
    console.error('Error toggling hit area visibility:', error)
  }
}

let moodTransitionTimeout: number | null = null
...
function onMoodChanged(data: { mood: string; animation: string }) {
  ...
  if (moodTransitionTimeout !== null) {
    window.clearTimeout(moodTransitionTimeout)
    moodTransitionTimeout = null
  }
  
  try {
    if (typeof (viewerRef.value as any).playAnimationSequence === 'function') {
      if (motionAnim && data.animation === 'idle2') {
        // Play motion sequence: motion first, then idle2
        (viewerRef.value as any).playAnimationSequence([motionAnim, data.animation])
      } else {
        // For idle1 or no motion, just play the animation
        (viewerRef.value as any).playAnimationSequence([data.animation])
      }
    } else {
      // Fallback: just play the animation directly
      console.warn('playAnimationSequence not available')
      store.selectedAnimation = data.animation
    }
    
    // Reload hit areas for the new idle state
    if (typeof (viewerRef.value as any).reloadHitAreas === 'function') {
      (viewerRef.value as any).reloadHitAreas()
    }
  } catch (error) {
